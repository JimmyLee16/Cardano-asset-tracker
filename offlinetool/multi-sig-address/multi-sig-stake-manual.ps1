<#
PowerShell Manual Cardano Multisig Stake Address Flow with Interactive Navigation
Bilingual Support: English & Vietnamese
- Creates multisig stake addresses using shared derivation path (1854H/1815H/0H/2)
- Supports M-of-N signatures for stake operations
...
$script:strings = @{
    en = @{
        ...
        chooseIndexMethod = "Choose Stake index method:"
        ...
        enterStakeIndex = "Enter Stake key index (0-2147483647) [default: 0]"
        ...
        derivingStake = "  → Deriving Stake key (1854H/1815H/0H/0/{0})..."
        exportingPublic = "  → Exporting public key..."
        calculatingHash = "  → Calculating key hash..."
        generatingStakeAddr = "  → Generating individual Stake address..."
        keysDerived = "Keys derived for: {0}"
        keyHash = "    Key hash: {0}"
        StakeAddr = "    Stake address: {0}"
        ...
        generatingAddress = "Generating multisig Stake address..."
        ...
        file3 = "  🔐 {0}.pay.{1}.xsk - Stake private key"
        file4 = "  🔓 {0}.pay.{1}.xvk - Stake public key"
        file5 = "  🔑 {0}.hash       - Key hash"
        file6 = "  💳 {0}.Stake.addr - Individual Stake address"
        policyFiles = "Multisig Policy Files:"
        file7 = "  📜 policy.txt         - Policy expression"
        file8 = "  🆔 policy_id.txt      - Policy ID"
        file9 = "  ⭐ multisig.addr      - Multisig Stake address (use this!)"
        ...
        useIndividualAddr = "✓ Each participant can use their .Stake.addr for individual transactions (1 signature)"
    ...
    vi = @{
        ...
        chooseIndexMethod = "Chọn phương thức nhập Stake index:"
        ...
        enterStakeIndex = "Nhập Stake key index (0-2147483647) [mặc định: 0]"
        ...
        derivingStake = "  → Đang tạo Stake key (1854H/1815H/0H/0/{0})..."
        exportingPublic = "  → Đang xuất public key..."
        calculatingHash = "  → Đang tính key hash..."
        generatingStakeAddr = "  → Đang tạo địa chỉ Stake cá nhân..."
        keysDerived = "Đã tạo key cho: {0}"
        keyHash = "    Key hash: {0}"
        StakeAddr = "    Địa chỉ Stake: {0}"
        ...
        policyId = "Policy ID: {0}"
        generatingAddress = "Đang tạo multisig Stake address..."
        multisigAddress = "Multisig Address: {0}"
        ...
        file4 = "  🔓 {0}.pay.{1}.xvk - Stake public key"
        file5 = "  🔑 {0}.hash       - Key hash"
        file6 = "  💳 {0}.Stake.addr - Địa chỉ Stake cá nhân"
        policyFiles = "File Multisig Policy:"
        file7 = "  📜 policy.txt         - Biểu thức policy"
        file8 = "  🆔 policy_id.txt      - Policy ID"
        file9 = "  ⭐ multisig.addr      - Multisig Stake address (dùng cái này!)"
        ...
        useIndividualAddr = "✓ Mỗi người có thể dùng .Stake.addr để giao dịch cá nhân (1 chữ ký)"
...
function Step-DeriveKeys {
    ...
    foreach ($participant in $script:participants) {
        ...
        switch ($methodChoice) {
            "1" {
                # Manual index input
                $indexInput = Read-Host (Get-Text "enterStakeIndex")
                $participant.StakeIndex = if ([string]::IsNullOrWhiteSpace($indexInput)) { "0" } else { $indexInput }
            }
            "2" {
                ...
                if (Test-Path $addressFile) {
                    ...
                    try {
                        ...
                        $participant.StakeIndex = "0"  # Default index since we're using the actual value
                    } catch {
    ...
    foreach ($participant in $script:participants) {
        ...
        # Derive stake key (1854H shared path with stake purpose)
        Write-Host ((Get-Text "derivingStake") -f $participant.StakeIndex)
        $stakePath = "1854H/1815H/0H/2/$($participant.StakeIndex)" # Use /2 for stake purpose
        $stakeFile = Join-Path $script:keysDir "$name.stake.$($participant.StakeIndex).xsk"
        $rootKeyContent = Get-Content $rootFile -Raw
        $stakeKey = $rootKeyContent | & $script:cardanoExe key child $stakePath
        if ($LASTEXITCODE -ne 0) {
            Write-Error "Failed to derive stake key for $name"
            return $false
        }
        Set-Content -Path $payFile -Value $payKey -NoNewline -Encoding ASCII
        
        # Export stake public key
        Write-Host (Get-Text "exportingPublic")
        $xvkFile = Join-Path $script:keysDir "$name.stake.$($participant.StakeIndex).xvk"
        $stakeKeyContent = Get-Content $stakeFile -Raw
        $pubKey = $stakeKeyContent | & $script:cardanoExe key public --without-chain-code
        if ($LASTEXITCODE -ne 0) {
            Write-Error "Failed to export stake public key for $name"
            return $false
        }
        ...
        # Generate individual stake address for this participant
        Write-Host (Get-Text "generatingStakeAddr")
        $stakeAddr = $xvkContent | & $script:cardanoExe address stake --network-tag $script:networkTag
        if ($LASTEXITCODE -ne 0) {
            Write-Warning "Failed to generate stake address for $name"
            $stakeAddr = $null
        } else {
            $stakeAddr = $stakeAddr.Trim()
            $stakeAddrFile = Join-Path $script:keysDir "$name.stake.addr"
            Set-Content -Path $stakeAddrFile -Value $stakeAddr -Encoding UTF8
        }
        
        Write-Host ((Get-Text "keysDerived") -f $name) -ForegroundColor Green
        Write-Host ((Get-Text "keyHash") -f $keyHash) -ForegroundColor Cyan
        if ($StakeAddr) {
            Write-Host ((Get-Text "StakeAddr") -f $StakeAddr) -ForegroundColor Magenta
        }
        
        $script:keyHashes += $keyHash
        $participant.KeyHash = $keyHash
        $participant.XvkFile = $xvkFile
        $participant.StakeAddr = $StakeAddr
    }
...
function Step-GenerateAddress {
    ...
    $script:multisigAddr = Get-Content "script.hash" | & $script:cardanoExe address Stake --network-tag $script:networkTag
    if ($LASTEXITCODE -ne 0) {
...
function Show-CompletionSummary {
    ...
    Write-Host "  🔐 1.pay.xsk    - Stake private key"
    Write-Host "  🔓 1.pay.xvk    - Stake public key"
    ...
    Write-Host "  🔐 2.pay.xsk    - Stake private key"
    Write-Host "  🔓 2.pay.xvk    - Stake public key"
    ...
    Write-Host "  ⭐ multisig.addr      - Multisig Stake address (dùng cái này!)`n"
    ...
    Write-Host "✓ Mỗi người có thể dùng .Stake.addr để giao dịch cá nhân (1 chữ ký)`n"
}
...
foreach ($participant in $script:participants) {
    ...
    Write-Host ((Get-Text "file3") -f $participant.Name, $script:StakeIndex)
    Write-Host ((Get-Text "file4") -f $participant.Name, $script:StakeIndex)
    Write-Host ((Get-Text "file5") -f $participant.Name)
    if ($participant.StakeAddr) {
        Write-Host ((Get-Text "file6") -f $participant.Name)
        Write-Host "     $($participant.StakeAddr)" -ForegroundColor Gray
    }
}
